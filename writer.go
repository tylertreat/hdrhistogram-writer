...
func WriteDistribution(hist *hdrhistogram.Histogram, percentiles Percentiles, writer io.Writer) error {
	if percentiles == nil {
	...
	for _, percentile := range percentiles {
		value := float64(hist.ValueAtQuantile(percentile)) / 1000000
...
// percentiles is nil, it defaults to a logarithmic percentile scale.
func WriteDistributionFile(hist *hdrhistogram.Histogram, percentiles Percentiles, file string) error {
	f, err := os.Open(file)
	if err != nil {
	...
	return WriteDistribution(hist, percentiles, f)
}
